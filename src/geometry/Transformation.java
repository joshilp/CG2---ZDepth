...
public class Transformation {
		...
		{		
			...
			for (int i=0; i<4; i++) 
			{
				for (int j=0; j<4; j++) 
				{
					...
					{
						if (flag == false){ 
							new_matrix[i][j] += a[i][k]* b[k][j];} //for premult
						else{
							new_matrix[i][j] += b[i][k]* a[k][j];} //for postmult
					}
				}
			}		
			return new_matrix;		
		...
		public Vertex3D getV3DTrans(Vertex3D a) 
		{		
		...
		{
//			double[][]copy_here = new double[N][N];
//			init_matrix(copy_here, 0); 
			for (int i = 0; i < N; i++)
			{
				for (int j = 0; j < N; j++)
				{
